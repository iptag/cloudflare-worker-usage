...
async function handleRequest(request) {
  ...
  if (path === '/tts') {
    ...
    const outputFormat = requestUrl.searchParams.get('o') || 'riff-24khz-16bit-mono-pcm';
          ...
          <div class="bg-white overflow-hidden shadow rounded-lg divide-y divide-gray-200">
            ...
            <div class="px-4 py-5 sm:p-6">
              ...
              <ul class="list-disc pl-5 text-sm space-y-1">
                ...
                <li>response_format: 输出格式 (wav, opus)</li>
              </ul>
...
async function getVoice(text, voiceName = 'zh-CN-XiaoxiaoMultilingualNeural', rate = 0, pitch = 0, style = 'general', outputFormat='riff-24khz-16bit-mono-pcm', download=false) {
  ...
  if(response.ok) {
    ...
    resp.headers.set('Content-Disposition', `attachment; filename="${uuid()}.wav"`);
    return resp;
  } else {
